...
uint16_t bufferCount[MAXMUS] = {0};
uint16_t maxValue = 0;
uint16_t minValue = 0;
uint64_t valuesBuffer[MAXMUS][BUFFER_SIZE];
uint64_t avgDelay[MAXMUS] = {0};
...
{
  // Check if it is the master MU svMUs
  for (size_t i = 0 ; i < svMUs; i++)
  {
    ...
    if (strcmp(SVSubscriber_ASDU_getSvId(asdu), svID[i]) == 0)
    {
      ...
      //PARTE PARA CALCULAR MÉDIA DE TEMPO ENTRE PACOTES:
      
      valuesBuffer[i][bufferCount[i]] = Hal_getTimeInMs();
      //if ((SVSubscriber_ASDU_getSmpCnt(asdu) == 4799))  printf("%d\n",Hal_getTimeInMs()-firstTs);
      if (bufferCount[i] > BUFFER_SIZE-1){
        avgDelay[i] = 0;
        for (uint32_t j=0; j<BUFFER_SIZE-1; j++){
          uint64_t diff = valuesBuffer[i][j+1] - valuesBuffer[i][j];
          avgDelay[i] += diff;
          if ((i==0)&&(diff > maxValue)) maxValue = diff;
        }
        printf("avgDelay[%d]: %d\n", i, avgDelay[i]);
        printf("Average delay between messages - %s (MU %d): %.4f us | Max: %.4f us\n", SVSubscriber_ASDU_getSvId(asdu), i, (avgDelay[i]*1000.0)/((BUFFER_SIZE-1)*1.0), (maxValue*1.0));
        bufferCount[i] = 0;
        maxValue = 0;
      } else {
        bufferCount[i]++;
      ...
      //if (SVSubscriber_ASDU_getSmpCnt(asdu) == 0) fprintf(stdout, "\x1b[32m %s -> IA: %10.2f   | IB: %10.2f   | IC: %10.2f | TripG: %d | AngleA: %f | AngleB: %f | AngleC: %f\x1b[0m\n",svID[i], ia_rms[4], ib_rms[4], ic_rms[4], TRIP_g, angle_a, angle_b, angle_c);
	//fprintf(stdout, "\x1b[32m %s -> TripA: %d   | TripB: %d   | TripC: %d | TripG: %d\x1b[0m\n", svID[i], TRIP_a, TRIP_b, TRIP_c, TRIP_g);
      ...
      */
      ...
      //Found the MU, break the "for" loop
      break;
    }
  }
}
